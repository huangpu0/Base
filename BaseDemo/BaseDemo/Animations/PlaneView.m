...
+ (instancetype)loadPlaneViewWithPoint:(CGPoint)point{
    PlaneView *plane = [[NSBundle mainBundle]loadNibNamed:@"PlaneView" owner:self options:nil].lastObject;
...
- (void)addAnimationsMoveToPoint:(CGPoint)movePoints endPoint:(CGPoint)endPoint{
    ...
    [self.layer addAnimation:animationGroup forKey:@"planeView"];
}
...
- (void)setPlaneScrewImages:(NSArray *)imageArray{
    ...
    for(int i = 1;i<4;i++){
        ...
        UIImage *planeImage = [UIImage imageNamed:imageStr];
