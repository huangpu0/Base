...
@interface SportsCarView ()<CAAnimationDelegate>
...
@implementation SportsCarView

+ (instancetype)loadCarViewWithPoint:(CGPoint)point{
    SportsCarView *carView = [[NSBundle mainBundle]loadNibNamed:@"SportsCarView" owner:self options:nil].lastObject;
    carView.frame = CGRectMake(point.x, point.y, 166, 70);
    [carView setPlaneScrewImages:nil];
    return carView;
}
...
- (void)setPlaneScrewImages:(NSArray *)imageArray{
    ...
    UIImage *yellowImage = [UIImage imageNamed:@"3"];
    UIImage *redImage = [UIImage imageNamed:@"1"];
